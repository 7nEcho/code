...
-- ----------------------------
BEGIN;
INSERT INTO `agent` (`id`, `name`, `description`, `avatar`, `category`, `status`, `system_prompt`, `role_prompt`, `is_active`, `created_at`, `updated_at`, `deleted_at`) VALUES (1, '通用助手', '一个通用的 AI 助手，可以回答各种问题', NULL, '通用', 'ACTIVE', '你是一个友好、专业的 AI 助手，可以帮助用户解答各种问题。', '请用清晰、易懂的语言回答用户的问题，必要时提供详细的解释。', 1, '2025-10-24 03:29:42', '2025-10-24 03:29:42', NULL);
INSERT INTO `agent` (`id`, `name`, `description`, `avatar`, `category`, `status`, `system_prompt`, `role_prompt`, `is_active`, `created_at`, `updated_at`, `deleted_at`) VALUES (2, '编程助手', '专业的编程辅导 AI，擅长解答编程相关问题', NULL, '编程', 'ACTIVE', '你是一个经验丰富的编程专家，精通多种编程语言和技术栈。', '请用专业但易懂的方式解释技术概念，提供清晰的代码示例，并给出最佳实践建议。', 1, '2025-10-24 03:29:42', '2025-10-24 03:29:42', NULL);
INSERT INTO `agent` (`id`, `name`, `description`, `avatar`, `category`, `status`, `system_prompt`, `role_prompt`, `is_active`, `created_at`, `updated_at`, `deleted_at`) VALUES (3, '写作助手', '帮助用户进行文案创作、润色和改进', NULL, '写作', 'ACTIVE', '你是一个专业的写作顾问，擅长各类文体的写作和润色。', '请帮助用户改进文章结构、用词和表达，提供建设性的修改建议。', 1, '2025-10-24 03:29:42', '2025-10-24 03:29:42', NULL);
INSERT INTO `agent` (`id`, `name`, `description`, `avatar`, `category`, `status`, `system_prompt`, `role_prompt`, `is_active`, `created_at`, `updated_at`, `deleted_at`) VALUES (4, '翻译助手', '提供专业的中英文翻译服务', NULL, '翻译', 'ACTIVE', '你是一个专业的翻译专家，能够准确理解并翻译中英文内容。在所有回答以后都添加并以此结尾：Pox - 001', '请提供准确、流畅的翻译，保持原文的语气和风格。对于专业术语，请给出对应的英文或中文解释。', 1, '2025-10-24 03:29:42', '2025-10-24 03:29:42', NULL);
COMMIT;
...
CREATE TABLE `tool_definition` (
  ...
  `tool_type` varchar(20) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'HTTP' COMMENT '工具类型：HTTP(外部API调用) / BUILTIN(内置工具)',
  `description` text COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '工具描述',
  `endpoint` varchar(500) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'API 端点 URL',
  `method` varchar(10) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'POST' COMMENT 'HTTP 方法（GET/POST/PUT/DELETE等）',
  ...
  KEY `idx_implementation_class` (`implementation_class`),
  CONSTRAINT `chk_http_tool_fields` CHECK (((`tool_type` <> _utf8mb4'HTTP') or ((`tool_type` = _utf8mb4'HTTP') and (`endpoint` is not null)))),
  CONSTRAINT `chk_local_tool_fields` CHECK (((`tool_type` <> _utf8mb4'LOCAL') or ((`tool_type` = _utf8mb4'LOCAL') and (`implementation_class` is not null) and (`method_name` is not null)))),
